...
The first implementation increment will cover the cancellation state machine and persistent retry-count budget while preserving the Stage 01-02 regression boundary. Execution-time accounting, process interleaving coverage, approval invalidation, and the complete AC-09..13 matrix remain open until implemented and tested.
